...
checkSystemForUpdate() {
    ...
    if ! command -v yq >/dev/null; then
        ...
        wget "https://gh.b52m.cn/https://github.com/mikefarah/yq/releases/latest/download/yq_linux_${arch}" -O /usr/bin/yq
        if [ $? -ne 0 ]; then
    ...
    case ${masquerade_type} in 
        ...
        "file")
            ...
            if [ ! -d "${masquerade_file}" ];then
                ...
                wget -q -O ./mikutap.tar.gz https://gh.b52m.cn/https://github.com/HFIProgramming/mikutap/archive/refs/tags/2.0.0.tar.gz
...
downloadHysteriaCore(){
    ...
    local arch=$(uname -m)
    local url_base="https://gh.b52m.cn/https://github.com/apernet/hysteria/releases/download/${version}/hysteria-linux-"
    local download_url=""
...
hihy_update_notifycation(){
	localV=${hihyV}
	remoteV=`curl -fsSL https://gh.b52m.cn/https://raw.githubusercontent.com/emptysuns/Hi_Hysteria/refs/heads/main/server/hy2.sh | sed  -n 2p | cut -d '"' -f 2`
	if [ -z $remoteV ];then
...
hihyUpdate(){
	...
	if [ "${localV}" = "${remoteV}" ];then
		...
		rm /usr/bin/hihy
		wget -q -O /usr/bin/hihy --no-check-certificate https://gh.b52m.cn/https://raw.githubusercontent.com/emptysuns/Hi_Hysteria/refs/heads/main/server/hy2.sh 2>/dev/null
...
install() {
    ...
    # 获取版本并下载核心
    version=$(curl --silent --head https://gh.b52m.cn/https://github.com/apernet/hysteria/releases/latest | grep -i location | grep -o 'tag/[^[:space:]]*' | sed 's/tag\///;s/ //g')
    checkSystemForUpdate
    downloadHysteriaCore
    setHysteriaConfig
