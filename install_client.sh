...
download_hysteria() {
  ...
  local _download_url="$speed_url$REPO_URL/releases/download/app/$_version/hysteria-$OPERATING_SYSTEM-$ARCHITECTURE"
...
# set client mode
sed -i '7s/server/client/1' /etc/systemd/system/hysteria-server.service
systemctl daemon-reload